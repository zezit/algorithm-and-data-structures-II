...
public class App {
    ...
    public static class PlayersQueue {
        ...
        private int firstElement = 0, lastElement = 0;
        private Jogador[] fifo;

        public PlayersQueue(int maxSize) {
            this.queueSize = maxSize;
            this.fifo = new Jogador[this.queueSize];
        }
        ...
        public void enqueue(Jogador jogador) {
            if (this.isFull()) {
                this.dequeue();
            }
        ...
        public Jogador dequeue() {
            if (this.isEmpty()) {
                return null;
            }

            Jogador jogador = this.fifo[this.firstElement];

            this.firstElement = (this.firstElement + 1) % this.queueSize;
        ...
        public double obterMediaAltura() {
            double media = 0;
            int i = this.firstElement, size = 0;

            while (i != this.lastElement) {
                media += this.fifo[i].getAltura();
                i = (i + 1) % this.queueSize;
                size++;
            }
        ...
        private boolean isFull() {
            if (this.firstElement == 0 && this.lastElement == this.queueSize - 1) {
                return true;
            }
            if (this.firstElement == this.lastElement + 1) {
                return true;
            }
            return false;
        }
